...
public class PostgreSQLDialectAdapter implements DialectAdapter {
    ...
    private static final Set<String> TIMEOUT_SQL_STATES = Sets.newHashSet(SQL_STATE_ER_QUERY_CANCELLED, SQL_STATE_ER_LOCK_TIMEOUT);

    // Information schema / System tables constants
    private static final ImmutableList<String> EXCLUDED_SCHEMAS = ImmutableList.of("information_schema");
    ...
    public ImmutableList<String> discoverTables(
        ...
        final String query = String.format("SELECT table_schema," +
                        "  table_name" +
        ...
        try (PreparedStatement stmt = dataSource.getConnection().prepareStatement(query)) {
            ...
            logger.warn("Transient connection error while discovering table list for datasource={}",
                    dataSource, e);
            schemaDiscoveryErrors.inc();
            throw new RetriableSchemaDiscoveryException(e);
        } catch (SQLNonTransientConnectionException e) {
            logger.error("Non Transient connection error while discovering table list for datasource={}",
                    dataSource, e);
            schemaDiscoveryErrors.inc();
    ...
    public ImmutableMap<String, ImmutableMap<String, SourceColumnType>> discoverTableSchema(
            ...
            throws SchemaDiscoveryException, RetriableSchemaDiscoveryException {
        logger.info("Discovering table schema for Datasource: {}, SourceSchemaReference: {}, tables: {}",
                dataSource, sourceSchemaReference, tables);

        final String query = String.format("SELECT column_name," +
                        "  data_type," +
        ...
        try (PreparedStatement statement = dataSource.getConnection().prepareStatement(query)) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    ...
                    while (resultSet.next()) {
                        ...
                        if (typeHasMaximumCharacterLength) {
                            sourceColumnType = new SourceColumnType(columnType, new Long[]{characterMaximumLength}, null);
                        } else if (typeHasPrecision && typeHasScale) {
                            sourceColumnType = new SourceColumnType(columnType, new Long[]{numericPrecision, numericScale}, null);
                        } else if (typeHasPrecision) {
                            sourceColumnType = new SourceColumnType(columnType, new Long[]{numericPrecision}, null);
                        } else {
            ...
            logger.warn("Transient connection error while discovering table schema for datasource={} db={} tables={}",
                    dataSource, sourceSchemaReference, tables, e);
            schemaDiscoveryErrors.inc();
            throw new RetriableSchemaDiscoveryException(e);
        } catch (SQLNonTransientConnectionException e) {
            logger.error("Non Transient connection error while discovering table schema for datasource={}, db={} tables={}",
                    dataSource, sourceSchemaReference, tables, e);
            schemaDiscoveryErrors.inc();
            throw new SchemaDiscoveryException(e);
        } catch (SQLException e) {
            logger.error("Sql exception while discovering table schema for datasource={} db={} tables={}",
                    dataSource, sourceSchemaReference, tables, e);
            schemaDiscoveryErrors.inc();
            throw new SchemaDiscoveryException(e);
        ...
        logger.info("Discovered tale schema for Datasource: {}, SourceSchemaReference: {}, tables: {}, schema: {}",
    ...
    public ImmutableMap<String, ImmutableList<SourceColumnIndexInfo>> discoverTableIndexes(
            ...
            ImmutableList<String> tables)
            throws SchemaDiscoveryException, RetriableSchemaDiscoveryException {
        logger.info("Discovering Indexes for DataSource: {}, SourceSchemaReference: {}, Tables: {}",
                dataSource, sourceSchemaReference, tables);

        final String query = String.format("SELECT a.attname as column_name," +
        ...
        try (PreparedStatement statement = dataSource.getConnection().prepareStatement(query)) {
            for (String table : tables) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {
                        indexInfosBuilder.add(SourceColumnIndexInfo
                                .builder()
            ...
            logger.warn("Transient connection error while discovering table indexes for datasource={} db={} tables={}",
                    dataSource, sourceSchemaReference, tables, e);
            schemaDiscoveryErrors.inc();
            throw new RetriableSchemaDiscoveryException(e);
        } catch (SQLNonTransientConnectionException e) {
            logger.error("Non Transient connection error while discovering table indexes for datasource={}, db={} tables={}",
                    dataSource, sourceSchemaReference, tables, e);
            schemaDiscoveryErrors.inc();
            throw new SchemaDiscoveryException(e);
        } catch (SQLException e) {
        ...
        logger.info("Discovered Indexes for DataSource: {}, SourceSchemaReference: {}, Tables: {}.\nIndexes: {}",
    ...
    public String getCountQuery(String tableName, ImmutableList<String> partitionColumns, long timeoutMillis) {
        return addWhereClause(
                String.format("SET statement_timeout = %s; SELECT COUNT(*) FROM %s", timeoutMillis, tableName),
                partitionColumns);
    }

    @Override
    public String getBoundaryQuery(String tableName, ImmutableList<String> partitionColumns, String colName) {
        return addWhereClause(String.format("SELECT MIN(%s), MAX(%s) FROM %s", colName, colName, tableName), partitionColumns);
    }
    ...
    private String addWhereClause(String query, ImmutableList<String> partitionColumns) {
        ...
        if (!partitionColumns.isEmpty()) {
            ...
            queryBuilder.append(partitionColumns
                    .stream()
                    // Include the column / range to define the where clause.
                    // `(exclude col = FALSE) OR (col >= range.start() AND (col < range.end() OR (range.isLast() = TRUE AND col = range.end()))`
                    .map(partitionColumn -> String.format(
    ...
    // Ref <a href="https://www.postgresql.org/docs/16/catalog-pg-type.html#CATALOG-TYPCATEGORY-TABLE"></a>
