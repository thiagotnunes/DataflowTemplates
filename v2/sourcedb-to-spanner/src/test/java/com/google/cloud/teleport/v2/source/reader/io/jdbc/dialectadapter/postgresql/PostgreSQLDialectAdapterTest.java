...
public class PostgreSQLDialectAdapterTest {
    ...
    @Test
    public void testDiscoverTablesReturnsSchemaAndTableNames() throws SQLException, RetriableSchemaDiscoveryException {
    ...
    public void testDiscoverTableSchema() throws SQLException, RetriableSchemaDiscoveryException {
        ...
        when(mockResultSet.wasNull()).thenReturn(
                // id column -> character_maximum_length == null, numeric_precision != null, numeric_scale == null
                true, false, true,
                // col column -> character_maximum_length == null, numeric_precision == null, numeric_scale == null
                true, true, true);

        assertThat(adapter.discoverTableSchema(mockDataSource, sourceSchemaReference, tables))
                .containsExactly("my_schema.table1", ImmutableMap.of(
    ...
    public void testDiscoverTableIndexes() throws SQLException, RetriableSchemaDiscoveryException {
        ...
        when(mockResultSet.getString("index_name")).thenReturn("table_pkey", "table_uniq", "table_index", "table_index");
        ...
        assertThat(adapter.discoverTableIndexes(mockDataSource, sourceSchemaReference, tables))
                .containsExactly("my_schema.table1", ImmutableList.of(
    ...
    public void testCheckForTimeout() {
        assertThat(adapter.checkForTimeout(new SQLException("Expected test non-timeout error"))).isFalse();
        // SQLState: IO error
        assertThat(adapter.checkForTimeout(new SQLException("Expected test non-timeout error", "58030"))).isFalse();

        // SQLState: Query cancelled
        assertThat(adapter.checkForTimeout(new SQLException("Expected test timeout error", "57014"))).isTrue();
        // SQLState: Lock timeout
        assertThat(adapter.checkForTimeout(new SQLException("Expected test timeout error", "55P03"))).isTrue();
    }
}
